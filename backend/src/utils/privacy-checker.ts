...
class PrivacyViolationDetector {
  static SENSITIVE_PATTERNS: Record<string, PatternConfig> = {
   ...
   * @param data - Data to check (object, string, or JSON)
   * @returns Array of violations found
   */
  static check(data: unknown): Violation[] {
    ...
    if (typeof data === 'string') {
      ...
      try {
        ...
        return [{ type: 'error', description: 'Failed to serialize data for privacy check', severity: 'ERROR', count: 0, examples: [], location: 'root' }];
      }
    } else {
      dataStr = String(data);
    }
   ...
   */
  static maskSensitiveData(data: string, type: string): string {
   ...
   */
  static findLocation(data: unknown, match: string): string {
    ...
    function search(obj: Record<string, unknown>, path: string[] = []): void {
   ...
   */
  static assertSafe(data: unknown, context: string = 'data'): boolean {
   ...
   */
  static getPrivacyScore(data: unknown): number {
    const violations = this.check(data);
   ...
   */
  static generateReport(data: unknown, context: string = 'data'): PrivacyReport {
    const violations = this.check(data);
