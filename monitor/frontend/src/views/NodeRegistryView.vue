        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 dark:border-slate-700">
              ...
              <th class="text-center py-2 px-3 text-gray-600 dark:text-slate-400 font-medium">Verify</th>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-slate-800">
            <tr v-for="node in sortedNodes" :key="node.address" class="hover:bg-gray-50 dark:hover:bg-slate-800/50">
              ...
              <td class="py-3 px-3 text-center">
                <button
                  @click="verifyOnChain(node.address)"
                  :disabled="verifyingAddress === node.address"
                  class="p-1 rounded hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors disabled:opacity-50"
                  :title="getVerifyTitle(node.address)"
                >
                  <svg v-if="verifyingAddress === node.address" class="w-4 h-4 animate-spin text-cyan-500" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
                  <svg v-else-if="verifiedNodes.get(node.address) === true" class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <svg v-else-if="verifiedNodes.get(node.address) === false" class="w-4 h-4 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <svg v-else class="w-4 h-4 text-gray-400 dark:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const verifyingAddress = ref<string | null>(null);
const verifiedNodes = ref<Map<string, boolean>>(new Map());
...
async function verifyOnChain(address: string) {
  verifyingAddress.value = address;
  try {
    const result = await nodeRegistry.checkRegistration(address);
    verifiedNodes.value.set(address, result?.isRegistered ?? false);
  } catch {
    verifiedNodes.value.set(address, false);
  } finally {
    verifyingAddress.value = null;
  }
}

function getVerifyTitle(address: string): string {
  const status = verifiedNodes.value.get(address);
  if (status === true) return 'Verified: Registered on-chain';
  if (status === false) return 'Verified: NOT registered on-chain';
  return 'Click to verify on-chain registration';
}
