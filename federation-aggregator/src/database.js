...
class AggregatorDatabase {
  ...
  getInvoiceCounts() {
    const total = this.db.prepare('SELECT COUNT(*) as count FROM aggregated_invoices').get().count;
  ...
  getDistribution(column, table = 'aggregated_tasks') {
    const stmt = this.db.prepare(`
